...
def main(file):
    ...
    for i, game in enumerate(steam_data):
        ...
        game_list[i][
            "totalTime"
        ] = f"{round(game.get('playtime_forever', 0) / 60, 2)} 小时"
        game_list[i]["lastTime"] = (
            time.strftime("%Y-%m-%d", time.localtime(game.get("rtime_last_played", 0)))
        ...
        game_list[i][
            "achievement"
        ] = f"{round(num / len(ach_data) * 100, 2) if ach_data else 0}%"
